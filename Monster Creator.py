...
def show_entry_fields():
   ...
   strmod = int(strength_score.get())
   strmod=math.floor((strmod-10)/2)
   
   dexmod = int(dexterity_score.get())
   dexmod=math.floor((dexmod-10)/2)
   
   conmod = int(constitution_score.get())
   conmod=math.floor((conmod-10)/2)
   
   wismod = int(wisdom_score.get())
   wismod=math.floor((wismod-10)/2)
   
   intmod = int(intelligence_score.get())
   intmod=math.floor((intmod-10)/2)
   
   chamod = int(charisma_score.get())
   chamod=math.floor((chamod-10)/2)
   
   filepath = os.path.join('c:/users/clay/documents/openhousemonsters', creature_name.get()+".txt")
   ...
   f.write(">|%s (%s)|%s (%s)|%s (%s)|%s (%s)|%s (%s)|%s (%s)|\n" % (strength_score.get(),strmod, dexterity_score.get(),dexmod, constitution_score.get(),conmod,intelligence_score.get(),intmod,wisdom_score.get(),wismod,charisma_score.get(),chamod))
   f.write("___\n")
